...
HERMES_HOME="$(env_file_get HERMES_HOME .env 2>/dev/null || echo "$HOME/.hermes")"
export HERMES_HOME
...
env_file_set CLIPROXY_API_KEY "$(env_file_get CLIPROXY_API_KEY .env)" "$penv"
env_file_set CLAUDE_MAX_PROXY_API_KEY local "$penv"
bu="$(env_file_get BROWSER_USE_API_KEY .env 2>/dev/null || true)"
...
if [[ -n "${PROFILE_DISCORD_BOT_TOKEN:-}" ]]; then
  ...
  env_file_set DISCORD_ALLOWED_USERS "${PROFILE_DISCORD_ALLOWED_USERS:-$(env_file_get DISCORD_ALLOWED_USERS .env 2>/dev/null || true)}" "$penv"
...
Profile '$name' is ready.
  ...
  Knowledge:   put .md/.txt files in $pdir/knowledge/ then, in chat:
